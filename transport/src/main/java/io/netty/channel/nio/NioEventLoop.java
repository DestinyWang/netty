...
public final class NioEventLoop extends SingleThreadEventLoop {
    ...
    /**
     * 对应服务端与客户端的连接过程
     * 类似于服务端 while(true) 来监听客户端连接并返回消息
     * 客户端 while(true) 监听服务端返回消息并处理
     *
     * 如果类型为 SELECT, 对应 {@link ServerSocket#accept()} 以及 {@link Socket#getOutputStream()}
     */
    @Override
    protected void run() {
        for (;;) {
    ...
    private void processSelectedKey(SelectionKey k, AbstractNioChannel ch) {
        ...
        try {
            ...
            if ((readyOps & (SelectionKey.OP_READ | SelectionKey.OP_ACCEPT)) != 0 || readyOps == 0) {
                // 如果是 [读取数据]的通知或者是[建立新连接]的通知
                // read() 方法有两个实现: NioByteUnsafe.read() 和 NioMessageUnsafe.read()
                // NioByteUnsafe 对应客户端 Channel 数据流读写的事件
                // NioMessageUnsafe 对应是否有新连接建立的事件
                unsafe.read();
            }
